    ...
    int wheel_indices[], uint8_t sieve_results[], uint32_t sieve_results_size)
{
    ...
    static constexpr int sieve_size = 32768;  //this is the size of the sieve segment in bytes.  todo: find optimum sieve size
    uint32_t segment_size = sieve_size * 30;

    __shared__ uint8_t sieve[sieve_size];
    ...
    for (int s = 0; s < segments; s++)
    {
        ...
        __syncthreads();
        //copy the sieve results to global memory and reset the sieve
        for (int j = 0; j < sieve_size; j++)
        {
