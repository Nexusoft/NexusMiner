...
namespace nexusminer {
    namespace gpu {
        ...
        //Return R mod m where m is the modulus (this), R is 2^BITS 
        //This is a special case of the classical modular reduction/division algorithm
        template<int BITS>
        __device__ void Cump<BITS>::R_mod_m(Cump& R) const
        {
            const bool divide_debug = true;  //enable printfs
            Cump<BITS> y = *this;  //divisor
            ...
            //t is the highest non-0 word of the divisor
            int t;
            for (t = y.LIMBS - 1; t >= 0; t--)
            {
                if (y.m_limbs[t] > 0)
                    break;
            }
            //normalize by shifting both dividend and divisor left until the MSB of the first divisor word is 1.
            int normalize_shift = __clz(y.m_limbs[t]);  //clz is an nvidia primitive that counts the leading zeros of a word
            ...
            if (divide_debug) printf("n %i t %i normalize_shift %i \n", n, t, normalize_shift);
            //after normalization, x is always smaller than normalized y, so no need to reduce x
            //Cump<BITS> temp = y << (32 * (n - t));
            //while (x >= temp)
            //{
            //    //quotient.m_limbs[n - t] += 1;
            //    x -= temp;
            ...
            uint32_t quotient_word;  //we don't need to store the entire quotient, just the current word.
            //step 3
            for (auto i = n; i > t; i--)  //t can be 0.  i is >= 1;
            {
            ...
            R = x >> normalize_shift;  //denormalize the remainder
