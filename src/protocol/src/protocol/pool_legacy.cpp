...
Pool_legacy::Pool_legacy(std::shared_ptr<stats::Collector> stats_collector)
...
void Pool_legacy::reset()
{
    m_current_height = 0;
}

network::Shared_payload Pool_legacy::login(std::string const& account_name, Login_handler handler)
{
...
network::Shared_payload Pool_legacy::get_work()
{
...
network::Shared_payload Pool_legacy::submit_block(std::vector<std::uint8_t> const& block_data,
...
network::Shared_payload Pool_legacy::extract_nbits_from_block(network::Shared_payload data, std::uint32_t& nbits)
{
