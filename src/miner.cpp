...
bool Miner::init()
{
	...
	m_wallet_connection = std::make_unique<Wallet_connection>(m_network_component->get_socket_factory()->create_socket(local_endpoint));
...
void Miner::run()
{
	...
	auto result = m_wallet_connection->connect(wallet_endpoint);
